...
int main() {
    try {
        ...
        io.outputstring("Additionally, 1800 MHz band provides another 10 MHz with 30 users per 1 MHz.\n");
        int main_total_khz = 1000;

        int mainSub = sim.mainSubchannelsFor(main_total_khz);
        io.outputstring("Main subchannels (10 kHz) available: "); io.outputint(mainSub); io.terminate();

        io.outputstring("1800 MHz high band: "); io.outputint(sim.highBandMHz()); io.outputstring(" MHz available (10 MHz total).\n");
        io.outputstring("High band users per 1 MHz (base): "); io.outputint(FiveGSimulator::USERS_PER_1MHZ_HIGH); io.terminate();
        ...
        io.outputstring("\nCalculations:\n");
        io.outputstring(" Main band capacity = main_subchannels * (users_per_10kHz_main * antennas) = "); io.outputint(mainSub); io.outputstring(" * ("); io.outputint(FiveGSimulator::USERS_PER_10KHZ_MAIN); io.outputstring(" * "); io.outputint(antennas); io.outputstring(") = "); io.outputint(mainCap); io.terminate();

        io.outputstring(" High band capacity = high_band_MHz * (users_per_1MHz_high * antennas) = "); io.outputint(sim.highBandMHz()); io.outputstring(" * ("); io.outputint(FiveGSimulator::USERS_PER_1MHZ_HIGH); io.outputstring(" * "); io.outputint(antennas); io.outputstring(") = "); io.outputint(highCap); io.terminate();

        io.outputstring(" Total combined capacity = main + high = "); io.outputint(mainCap); io.outputstring(" + "); io.outputint(highCap); io.outputstring(" = "); io.outputint(totalCap); io.terminate();
        ...
        if (n < 0) n = 0;
        if (n > totalCap) {
            io.outputstring("Note: provided device count exceeds theoretical total capacity; only first "); io.outputint(totalCap); io.outputstring(" will be considered.\n");
            n = totalCap;
        }
        ...
        // per-device assignment reporting
        ...
        for (int i = 0; i < n; ++i) {
            ...
            io.outputstring(" Device "); io.outputint(i+1); io.outputstring(": ID="); io.outputint(devices[i]);
            if (band == 1) {
                io.outputstring(", assigned to MAIN band subchannel="); io.outputint(indexInBand);
                io.outputstring(", slot="); io.outputint((i % perMainCap) + 1);
            } else {
                io.outputstring(", assigned to HIGH 1800MHz band chunk(MHz)="); io.outputint(indexInBand);
                io.outputstring(", slot="); io.outputint(((i - mainCap) % perHighCap) + 1);
            }
            io.terminate();
        }
        ...
        io.outputstring("\nTotal messages required = n * messages_per_device = "); io.outputint(n); io.outputstring(" * "); io.outputint(FiveGSimulator::MESSAGES_PER_DEVICE); io.outputstring(" = "); io.outputint((int)totalMessages); io.terminate();
